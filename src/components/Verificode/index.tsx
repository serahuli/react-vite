...
const VerifiCode:FC<IVerifiCodeProps> = (props) => {
  ...
  const LETTERS = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'o', 'p', 'q', 'n', 'm', 'l', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
  const NUMLETTER = [...NUMBERS, ...LETTERS]

  const WIDTH: number = props.width ?? 100;
  const HEIGHT: number = props.height ?? 36;
  const CODETYPE: 'number' | 'letter' | 'numLetter' | 'defined' = props.codeType ?? 'number';
  ...
  const drawBgColor: () => void = () => {
    const canvas: HTMLCanvasElement = document.getElementById('verifiCode') as HTMLCanvasElement;
    const ctx: CanvasRenderingContext2D = canvas.getContext && canvas.getContext('2d') as CanvasRenderingContext2D;
    // 画背景颜色
    ctx.beginPath();
